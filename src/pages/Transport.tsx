...
import { 
  ...
  Activity, X, CheckCircle, Zap, Gauge, Fuel, PlayCircle, Smartphone, 
  FileText, Pencil, Trash2, ShieldCheck, Paperclip, AlertCircle, 
  Battery, Satellite, Lock, UserCheck, History, CreditCard, Calendar 
} from 'lucide-react';
        ...
        const interval = setInterval(() => {
           setVehicles(prevVehicles => prevVehicles.map(v => {
              if (v.gps && v.status === 'active') {
                 // Initialize heading if missing (migration)
                 ...
                 if (isMoving) {
                     ...
                     // Change heading slightly (simulating steering) if moving
                     if (newSpeed > 0) {
                         const turnAngle = (Math.random() - 0.5) * 10; // Turn up to 5 degrees left/right
                         newHeading = (currentHeading + turnAngle) % 360;
                     }
                 }

                 // Calculate new position based on speed and heading
                 // Factor 0.000002 roughly maps speed to lat/lng degrees per update
                        ...
                        {(() => {
                           ...
                           return (
                              <>
                                 ...
                                 <div className="flex items-center gap-2 text-xs text-gray-500 bg-gray-100 p-2 rounded-lg">
            ...
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
               ...
               <form onSubmit={handleAddMaintenance} className="p-6 space-y-4">
                  ...
                  <div>
                      ...
                      <select className="w-full p-2 rounded-lg border border-gray-300" value={newMaint.type} onChange={e => setNewMaint({...newMaint, type: e.target.value})}>
